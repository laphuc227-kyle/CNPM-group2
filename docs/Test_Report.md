...
- **Tested by:** [names]
- **Reference:** see `Testing Document.xlsx` for the full list of 30 test cases
...
- Admin – Member management (add / edit / delete / list)
- Admin – Book management (add / edit / delete / list)
...
- Manual tests: console menu walkthrough (see `Testing Document.xlsx`, TC01–TC30)
...
All 30 automated tests pass against a freshly-seeded `data/sample_data.json`
(1 member, 2 books, 1 active borrow record, no fines — see the file for the
exact seed). Manual test cases TC01–TC30 in `Testing Document.xlsx` were
...
and all 30 manual test cases in `Testing Document.xlsx` pass against the
...
info, book search, borrowed books, borrow history, fines), and admin
management of members, books, borrowing/returning, and fines — including
automatic overdue detection and fine calculation on return, matching the
Class Diagram's `Fine` entity design (persisted `fine_id` / `record_id` /
`amount` / `status`).
